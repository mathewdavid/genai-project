...
def configure_genai(api_key):
    if GENAI_AVAILABLE:
        ...
        st.error("Google Generative AI library is not available. Please install it to use this feature.")
...
def input_pdf_text(uploaded_file):
    if not PDF_AVAILABLE:
        st.error("PyPDF2 library is not available. Please install it to use this feature.")
        return None
    try:
...
def create_radar_chart(parsed_response):
    if not MATPLOTLIB_AVAILABLE:
        st.error("Matplotlib is not available. Please install it to use this feature.")
        return None
...
# Function for ATS Check with Job Description
